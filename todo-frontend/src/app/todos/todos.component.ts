...
export class TodosComponent implements OnInit {
  ...
  loadTodos(): void {
    ...
    this.todoService.getAll().subscribe({
      ...
      error: () => {
        this.ngZone.run(() => {
          this.message.error('Failed to load todos.');
          this.loading = false;
        });
      },
    });
  ...
  loadUsers(): void {
    this.userService.getAll().subscribe({
      next: (data) => this.ngZone.run(() => (this.users = data)),
  ...
  handleSubmit(): void {
    ...
    const { title, description, dueDate, userId } = this.todoForm.value;
    const formattedDate = dueDate instanceof Date ? dueDate.toISOString().split('T')[0] : dueDate;

    if (this.isEditMode && this.editingTodo) {
      this.todoService
        .update(this.editingTodo.id, { title, description, dueDate: formattedDate, userId })
        .subscribe({
          next: (updated) => {
            this.ngZone.run(() => {
      ...
      this.todoService.create({ title, description, dueDate: formattedDate, userId }).subscribe({
        next: (todo) => {
          this.ngZone.run(() => {
